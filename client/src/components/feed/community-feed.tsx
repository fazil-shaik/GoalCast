...
export function CommunityFeed({ filter = {}, searchTerm = "" }: CommunityFeedProps) {
  const [commentText, setCommentText] = useState("");
  const [activeFeedItem, setActiveFeedItem] = useState<FeedItemWithUser | null>(null);
  
  // Fetch feed items and online users
  ...
  const onlineUsers = onlineUsersQuery.data as { count: number; users: { id: number; name: string; avatarUrl?: string }[] } || { count: 0, users: [] };
  ...
  if (feedQuery.isLoading || onlineUsersQuery.isLoading) {
    return (
      <div>
        <div className="mb-6">
  ...
  return (
    <div>
        ...
        <div className="divide-y divide-gray-200">
          {filteredFeedItems.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-gray-400 mt-2">Try adjusting your search or create a new post.</p>
            </div>
          ) : (
      ...
      <div className="mt-6">
        <h4 className="text-sm font-medium text-gray-900 mb-3">Online Members</h4>
